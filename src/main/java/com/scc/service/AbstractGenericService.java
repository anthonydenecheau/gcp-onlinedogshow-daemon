...
import com.scc.model.SyncData;
import com.scc.repository.OdsDataRepository;
...
public abstract class AbstractGenericService<T> {
    ...
    protected SyncData saveDog(SyncData dog){
        try { return odsDataRepository.save(dog); }
        finally{ }
    }
    
    protected void deleteDog (SyncData dog) {
        try { odsDataRepository.delete(dog); }
        finally{ }
    }
    
    protected List<SyncData> getAllChanges(String _domaine){
        try { return odsDataRepository.findByTransfertAndDomaine("N", _domaine); }
        finally{ }
    }
